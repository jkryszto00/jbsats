                    ...
                    <template v-for="job in model.job_offers">
                        <div class="py-2 flex justify-between first:pt-0 last:pb-0">
                            ...
                            <div class="px-2 py-0.5 rounded bg-neutral-100 text-neutral-700">
                                <span v-if="job.salaries[0].from" class="uppercase">{{ formatter.format(job.salaries[0].from) }} - {{ formatter.format(job.salaries[0].to) }} {{ job.contract.currency }}</span>
                                <span v-else class="uppercase">{{ formatter.format(job.salaries[0].exact) }} {{ job.contract.currency }}</span>
                            </div>
                        </div>
                    </template>
                </div>
