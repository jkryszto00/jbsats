...
const prop = defineProps(['jobOffer', 'categories', 'levels', 'currencies', 'times', 'types', 'pers'])
...
const form = useForm({
    title: prop.jobOffer.title,
    description: prop.jobOffer.description,
    category: prop.jobOffer.categories,
    level: prop.jobOffer.level,
    contract: {
        currency: prop.jobOffer.contract.currency,
        time: prop.jobOffer.contract.time
    },
    salary: prop.jobOffer.salary
})

const submit = () => {
    form.patch(route('panel.posting.update', { jobOffer: prop.jobOffer }))
}
</script>
<template>
    <PanelLayout>
        <Card class="mx-auto max-w-2xl" title="Edit job offer">
            ...
            <template #footer>
                <div class="flex justify-end">
                    <Button type="submit" form="job-form">Save changes</Button>
                </div>
            </template>
        </Card>
    </PanelLayout>
